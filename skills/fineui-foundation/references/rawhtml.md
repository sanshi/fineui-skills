...
| | F.js | Pro (aspx) | Core-MVC (Fluent) | Core-TagHelper (标签) |
|--|------|-----------|-------------------|------------------------|
| 声明可信 HTML | `F.rawHtml('...')` | `XxxRawHtml="..."` | `.XxxRawHtml(new RawHtml("..."))` | `_XxxRawHtml="..."` 或 `XxxRawHtml="@(new RawHtml("..."))"` |
| 后置代码赋值 | — | `x.XxxRawHtml = new RawHtml(...)` | `x.XxxRawHtml = new RawHtml(...)` | `x.XxxRawHtml = new RawHtml(...)` |
| 消息框 | `showNotify(F.rawHtml(...))` / `F.alert({message: F.rawHtml(...)})` | `ShowNotify(new RawHtml(...))` | `ShowNotify(new RawHtml(...))` | `ShowNotify(new RawHtml(...))` |
...
4. **Core TagHelper 便捷形式**：markup 里直接写 HTML 字符串用 `_XxxRawHtml="..."`（下划线前缀）；含 Razor 变量/表达式用 `_XxxRawHtml="@(...)"` 或 `XxxRawHtml="@(new RawHtml(...))"`。
...
- [stacks.md](stacks.md)：五写法总览与命名规律
