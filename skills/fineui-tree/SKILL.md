...
description: >
  ...
  "EnableCheckBox", "复选框树", "CascadeCheck", "级联选中", "GetCheckedNodes", "getCheckedNodes",
  "懒加载", "OnNodeLazyLoad", "异步加载子节点", "节点点击", "nodeclick", "OnNodeExpand".
...
2. **节点从哪来？** 静态写死 / 后台代码构建（`Tree.Nodes.Add(new TreeNode{...})` 或 ViewBag）/ 异步懒加载。
3. **要不要复选框？** `checkbox`(F.js) / `EnableCheckBox`(C#) + 级联 `CascadeCheck`。
...
1. **先定写法、不混用**：F.js 节点属性 camelCase（`text`/`id`/`expanded`）；C# TreeNode PascalCase（`Text`/`NodeID`/`Expanded`）。F.js 用 `id`，C# 用 `NodeID`。
2. **级联选中是 `CascadeCheck`**：不是 `AutoCheckParent`/`AutoCheckChildNodes`（这些不存在）。
3. **读取选中节点用 `GetCheckedNodes()`**（返回 `TreeNode[]`）；没有 `GetCheckedNodeIDs`。F.js 客户端 `getCheckedNodes(true)`。
4. **读选中的方式随写法不同**：RazorForms 用控件字段 `Tree1.GetCheckedNodes()`；MVC/RazorPages 用客户端 `getCheckedNodes(true)` 序列化后回发 `JArray`。详见 [references/checkbox-events.md](references/checkbox-events.md)。
5. **懒加载用 `OnNodeLazyLoad` + `AutoLeafIdentification="false"` + 节点 `Leaf`**（不是 `EnableAjax`）；`Leaf=false` 的节点展开时触发懒加载，`Leaf=true` 为叶子。
